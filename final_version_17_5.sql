...
select kpi.* 
				, case when country = 'USA' and `Source.Name` like '%AMZ%' and `Source.Name` not like '%furn%'  then 'US_SG_AMZ'  
							 ...
							 when country = 'USA' and `Source.Name` like '%WM%' and Channel in ('WM WFS', 'WM DSV')  then 'Walmart'
							 when country ='USA' and `Source.Name` like '%fur%' and `Source.Name` not like '%WM%' then 'Furniture'
