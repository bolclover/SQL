...
,case when a.country = 'USA' and a.root_category = 'Sporting Goods' and a.Platform = 'AMZ' then 'US_SG_AMZ'  
		  when source_name like '%AMZ_USA%' then 'US_SG_AMZ'
			...
			when a.root_category = 'Furniture' and a.Platform in ('AMZ','WF') then 'Furniture'
			when source_name like '%fur%' then 'Furniture'
...
on a.sku = b.sku 
where company ='Y4A' and team ='YSL' and a.root_category in ('Sporting Goods','Furniture')
...
left join tran_table b 
on a.sku = b.sku 
where company ='Y4A' and team ='YSL' and a.root_category in ('Sporting Goods')
group by 1,2,3,4,5,6,7
) a 
left join fc_monthly b 
on a.log_date = b.date and a.sku = b.sku and a.country = b.country 
